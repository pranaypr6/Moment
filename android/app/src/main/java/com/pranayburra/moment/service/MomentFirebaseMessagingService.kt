package com.pranayburra.moment.service
...
import com.pranayburra.moment.R

@AndroidEntryPoint
class MomentFirebaseMessagingService : FirebaseMessagingService() {

    @javax.inject.Inject
    lateinit var deviceRepository: com.pranayburra.moment.domain.repository.DeviceRepository

    @javax.inject.Inject
    lateinit var relationshipRepository: com.pranayburra.moment.domain.repository.RelationshipRepository

    @javax.inject.Inject
    lateinit var notificationSettingsManager: com.pranayburra.moment.data.local.NotificationSettingsManager
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        if (data.containsKey("momentId") && data.containsKey("imageUrl")) {
            ...
            kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.IO).launch {
                try {
                    relationshipRepository.refreshCurrentRelationship()
                    com.pranayburra.moment.widget.RelationshipWidget.forceUpdate(applicationContext)
                } catch (e: Exception) {
                    Log.e("FCM", "Failed to refresh relationship", e)
                }
            }
