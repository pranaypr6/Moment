...
class MomentsViewModel @Inject constructor(
    ...
    private val api: com.pranayburra.moment.data.remote.MomentApi
) : ViewModel() {
    ...
    fun sendEmotionalAction(action: com.pranayburra.moment.ui.moments.EmotionalAction) {
        ...
        if (currentState is MomentsUiState.Success) {
            viewModelScope.launch {
                try {
                    ...
                    if (rel != null) {
                        val req = com.pranayburra.moment.data.remote.SendPresenceRequest(rel.id, typeInt)
